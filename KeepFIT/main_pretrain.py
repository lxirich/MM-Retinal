...
def process(args):
    ...
    dataloaders = get_loader(dataframes_path=args.dataframes_path, data_root_path=args.data_root_path,
                             ...
                             knowledge_dict= args.knowledge_dict)
...
def main():
    ...
    parser.add_argument('--dataframes_path', default=PATH_DATAFRAME_PRETRAIN)                       # 预处理好的数据pair
    ...
    parser.add_argument('--banned_categories', default=['myopia', 'cataract', 'macular hole', 'retinitis pigmentosa',
                                                        "myopic", "myope", "myop", "retinitis"])                # OOD类别实验，数据处理筛选
    parser.add_argument('--out_path', default=PATH_RESULTS_PRETRAIN+"keepfit/", help='output path')
    parser.add_argument('--caption', default="A [ATR] fundus photograph of [CLS]")                  # prompt模板  属性和类别
    parser.add_argument('--augment_description', default=True, type=lambda x: (str(x).lower() == 'true'))   # str=》bool  是否将类标变为医学描述
    parser.add_argument('--balance', default=False, type=lambda x: (str(x).lower() == 'true'))       # 是否平衡数据集 两种dataloader

    # 模型架构
    parser.add_argument('--architecture', default='resnet_v2', help='resnet_v1 -- efficientnet')    # img tower骨干网络
    parser.add_argument('--knowledge_dict', default=True, type=lambda x: (str(x).lower() == 'true'))  # 是否启用知识caption
    ...
    parser.add_argument('--weights_path', default='./results/pretraining/')  # 本地模型权重
    parser.add_argument('--load_weights', default=True, type=lambda x: (str(x).lower() == 'true'))  # 是否加载预训练权重

    # 设备
    parser.add_argument('--store_num', default=1, type=int)                                         # 保存间隔
    parser.add_argument('--num_workers', default=8, type=int, help='workers number for DataLoader') # cpu线程
    parser.add_argument("--local_rank", type=int, default=-1)

    args, unknown = parser.parse_known_args()                                                                    # 防止抛出异常
    process(args=args)
